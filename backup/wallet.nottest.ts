...
const { usdtABI } = require("../src/resources/usdtABI");
...
describe("Create Wallet From Mnemonic", () => {
    ...
    it("should create a wallet from a random mnemonic", async () => {
...
describe("Get Account Balance", () => {
    it("should get the Ether balance in the specified address through Infura API", async () => {
        const address = '0xC8F60EaF5988aC37a2963aC5Fabe97f709d6b357';
    ...
    it("should get the Ether balance in the specified new address through Infura API", async () => {
    ...
    it("should get the token contract balance in the specified address through Infura API", async () => {
        const address = '0xC8F60EaF5988aC37a2963aC5Fabe97f709d6b357';
        const usdtToken = '0x9e15898acf36C544B6f4547269Ca8385Ce6304d8';

        const balance = await wallet.getAccountTokenBalance(address, usdtToken, usdtABI);
        expect(ethers.formatUnits(balance, 6)).toBe("100.0");
    });

    it("should get the token contract balance in the specified new address through Infura API", async () => {
        const address = '0xE05eCB996dA9D59315d569D65C93Af68bA9AA4a5';
        const usdtToken = '0x9e15898acf36C544B6f4547269Ca8385Ce6304d8';
    ...
    it("should throw an error if the adderss is not valid", async () => {
        try {
        ...
        try {
            await wallet.getAccountTokenBalance();
        } catch (error) {
            // Assert that the error is thrown
            expect(error).toBeDefined();
            expect(error.message).toEqual("未指定钱包地址");
    ...
    it("should throw an error if the token ABI is not valid", async () => {
        const address = '0xC8F60EaF5988aC37a2963aC5Fabe97f709d6b357';
        try {
            await wallet.getAccountTokenBalance(address);
        } catch (error) {
            // Assert that the error is thrown
            expect(error).toBeDefined();
